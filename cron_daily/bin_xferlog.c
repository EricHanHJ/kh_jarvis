...
int main()
{
	...
	fr=fopen("/var/log/xferlog","r");
		...
		date[2]='\0';
		if(date[0]==' ') date[0]='0';
		for(i=0; i<8; i++)	full_time[i]=mydata[11+i];
		full_time[8]='\0';
...
void input_mysql(char *year, char *month, char *date, char *full_time, char *msg){
        ...
        sprintf(query, "insert into xferlog_log values('%s', '%s', '%s', '%s','%s')", year, month, date, full_time, msg);
