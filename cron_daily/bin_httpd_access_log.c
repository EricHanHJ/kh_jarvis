...
int main()
{
	...
	setuid(0);
	fr=fopen("/var/www/html/kh/httpd_access_log","r");
	if(fr == NULL)	exit(EXIT_FAILURE);
	while((read=getline(&line, &len, fr)) != -1){
		sp=index(line, '[');
		dp=rindex(line, '+');
		ddp=rindex(line, '"');
		strncpy(mydata,sp+1,dp-sp-2);
		strncpy(msg,dp+7,dp-sp+300);
		...
		date[1]=mydata[1];
		date[2]='\0';
		for(i=0; i<8 ;i++)	full_time[i]=mydata[12+i];
		full_time[8]='\0';
...
void input_mysql(char *year, char *month, char *date, char *full_time, char *msg){
	...
	sprintf(query, "insert into http_access_log values('%s', '%s', '%s', '%s','%s')", year, month, date, full_time, msg);
