...
int main()
{
	...
	char mydata[23] = {0x00,};
        ...
        setuid(0);
	fr=fopen("/var/www/html/kh/httpd_error_log","r");
        if(fr == NULL)
                exit(EXIT_FAILURE);
		...
		if(strchr(line, '[') || strchr(line, ']')){
                	...
                	dp=index(line, ']');
			ddp=rindex(line, '\0');
                	strncpy(mydata,sp+5,dp-sp-5);
			strncpy(msg,dp+3, dp-sp+300);
	               	//printf("[%s]\n", mydata);
...
void input_mysql(char *year, char *month, char *date, char *full_time, char *msg){
        ...
        sprintf(query, "insert into http_error_log values('%s', '%s', '%s', '%s','%s')", year, month, date, full_time, msg);
